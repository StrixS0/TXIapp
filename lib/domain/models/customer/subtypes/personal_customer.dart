...
class PersonalCustomer extends Customer{
  PersonalCustomer({required CustomerId id, required Profile profile, required Address address, required UserId userId, required Status status, required DateTime created, required DateTime modified}) : super(id: id, profile: profile, address: address, userId: userId, status: status, created: created, modified: modified);
}