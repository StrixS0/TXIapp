...
class CustomerFactoryImpl implements ICustomerFactory{
  ...
  Customer create({required String type, required Profile profile, required UserId userId, String? company}) {
    if(type == 'personal'){
      return PersonalCustomer(id: CustomerId(const Uuid().v1()), profile: profile, userId: userId, status: Status.inactive, created: DateTime.now(), modified: DateTime.now());
    }else if(type == 'corporate'){
      if(company == null ) throw DomainException({'company': 'Company is required.'});
      return CorporateCustomer(id: CustomerId(const Uuid().v1()), profile: profile, userId: userId, company: company, status: Status.inactive, created: DateTime.now(), modified: DateTime.now());
    }
    throw DomainException({'customer_type': 'Invalid customer type.'});
  }

  @override
  Customer reconstitute({required String type, required CustomerId id, required Profile profile, required UserId userId, String? company, required Status status, required DateTime created, required DateTime modified}) {
    if(type == 'personal'){
      return PersonalCustomer(id: id, profile: profile, userId: userId, status: status, created: created, modified: modified);
    }else if(type == 'corporate'){
      if(company == null ) throw DomainException({'company': 'Company is required.'});

      return CorporateCustomer(id: id, profile: profile, userId: userId, company: company, status: status, created: created, modified: modified);
    }
    throw DomainException({'customer_type': 'Invalid customer type.'});
  }
